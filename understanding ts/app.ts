...
enum Role { ADMIN = "ADMIN",STUDENT = 100,AUTHORITY = 120}
const person1 = {    name:'jp',
    age:40,
...
function combine(
    input1: Combinable,
    input2: Combinable, 
    resultconversation : Conversionable){
...
const u1:User = { name:'jeyaprakash',age:40}
console.log(u1.name)

type User1 = { name: string } | string;
let u11: User1 = {name: 'Max'};
console.log(u11)
u11 = 'Michael';
...
function add(n1:number,n2:number,cb: (r:number) => void ) {
    const result = n1+ n2;
    cb(result);
}
